...
const ImamSchedule = ({ user }) => {
  ...
  const fetchImages = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get('http://localhost:3001/api/imam');
  ...
  const uploadImage = async (file) => {
    ...
    const storageRef = ref(imgDB, `imam/${uniqueName}`);
  ...
  const handleImage = async (file) => {
    if (file) {
      ...
      try {
        ...
        const response = await axios.post('http://localhost:3001/api/imam', {
          imageUrl,
  ...
  const handleDelete = async (imageId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const token = localStorage.getItem('token');
          const response = await axios.delete(`http://localhost:3001/api/imam/${imageId}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-xl shadow-xl border border-gray-200 p-8">
            <div className="flex flex-col items-center justify-center min-h-[300px] text-center">
              ...
              <p className="mt-2 text-sm text-gray-500">
                {isAdmin 
                  ? "Upload a schedule image to get started"
