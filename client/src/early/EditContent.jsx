...
export default function EditActivityPage() {
  ...
  useEffect(() => {
    ...
    const fetchActivity = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/activitydetail/${id}`);
        console.log(response)
        const activity = response.data;
        setTitle(activity.title);
        setDescription(activity.description);
        setCategory(activity.category);
        setClassDateTime(activity.actdate || null);
        setClassType(activity.classType || '');
        setOnlineLink(activity.onlineLink || '');
        setClassFee(activity.classFee || '');
        setClassFeeAmount(activity.classFeeAmount || '');
        setImagePreviews(activity.media.filter(url => url.includes('.jpg') || url.includes('.png')));
        setVideoPreviews(activity.media.filter(url => url.includes('.mp4') || url.includes('.mov')));
      } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3001/activity/${id}`, activityData);
          ...
          <List>
            {navigationItems.map((item) => (
              ...
              >
                ...
                <Link
                  to={item.path}
                  style={{ textDecoration: 'none', color: location.pathname === item.path ? 'black' : 'white' }}
                >
